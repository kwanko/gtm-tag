...

{
  ...
  "version": 1,
  "securityGroups": [],
  "displayName": "Kwanko Default Tag",
  ...
  "description": "Kwanko default tag that shoud be setup on each pages of your website for cookieless tracking compatibility",
  "containerContexts": [
    "WEB"
  ]
}
...
[
  {
    "type": "TEXT",
    "name": "expirationDate",
    "displayName": "Cookie expiration date : must be matched with Kwanko custom JS variable \"expirationDate\"",
...
var kwkDebug;
kwkDebug = getCookieValues('kwkdebug').length > 0;
const log = kwkDebug ? logToConsole : (() => {});
var kwkUniv = getQueryParameters('kwkuniv', false);
log('KWKGTM debug - catched univ : '+kwkUniv);

if (getType(kwkUniv) === 'string' && kwkUniv.length > 0) {
  var cookieExpire = data.expirationDate;
  log('KWKGTM debug - cookieExpire : '+cookieExpire);
  var cookieOptions = {
  ...
  setCookie('kwku', decodeUriComponent(kwkUniv), cookieOptions);
  log('KWKGTM debug - set cookie kwkUniv with value '+kwkUniv+' with options :', cookieOptions);
}
